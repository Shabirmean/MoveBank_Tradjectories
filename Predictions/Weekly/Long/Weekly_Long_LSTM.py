...
for tortoise in ['Connor', 'Isabela', 'Miriam', 'Nathalie', 'Sepp']:
    ...
    # load the dataset
    dataframe = pandas.read_csv('../WeeklyRoutine/' + tortoise + '_Weekly_Long.csv', usecols=[1], engine='python',
                                header=None)
    ...
    f = open('./LongResults_Weekly/' + name_of_tortoise + '_objects.save', 'wb')
    for obj in [trainX, trainY, testX, testY]:
        cPickle.dump(obj, f, protocol=cPickle.HIGHEST_PROTOCOL)
    f.close()

    # Save the model generated
    model.save('./LongResults_Weekly/' + name_of_tortoise + '_model.h5')  # creates a HDF5 file 'my_model.h5'

    # save the model architecture as a JSON
    json_string = model.to_json()
    savePickleFile(json_string, './LongResults_Weekly/' + name_of_tortoise + '_json.json')
